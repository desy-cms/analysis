...
//const double obs = 8481.;
//const double obs = 38906.;	//Full re-reco old WP
const double obs = 26760.; 	//new WP

TH1* QCD_HistSyst(bool up, int iPar, double init_3[3], double para_a_array[3], double para_b_array[3], double dpara_b_array[3], TMatrixD para_b, TMatrixD eigenVa, double n)
{
  	TString outname; 
	...
	RooRealVar mbb("mbb","mbb", 500.0, 1700.0) ;
  	//RooRealVar peak("peak", "peak", para_a_array[0], 50.0, 500.0, "GeV");
  	//RooRealVar tail("tail", "tail", para_a_array[1], -1.0, 1.0);
  	//RooRealVar width("width", "width", para_a_array[2], 5.0, 850.0, "GeV");
	RooRealVar par_a("par_a", "par_a", para_a_array[0], 0.0, 10.0);
        RooRealVar par_b("par_b", "par_b", para_a_array[1], 0.02, 0.08);
        RooRealVar par_c("par_c", "par_c", para_a_array[2], 65., 150.); 
	mbb.setBins(48);
...
#endif
*/
  	//RooMyNovosibirsk novo("novosibirsk","novosibirsk PDF", mbb, peak, width, tail);
	//RooNovosibirsk novo("novosibirsk","novosibirsk PDF", mbb, peak, width, tail);
	RooGenericPdf dijet("dijet","dijet PDF","TMath::Exp(-par_a * TMath::Log(mbb/13000.) - par_b*par_a*(pow(TMath::Log(mbb/13000.),2)))*(-1+par_c*(mbb/13000.))",
	...
	TH1* h1 = dijet.createHistogram("QCD", mbb, Binning(48,500.,1700.));
...
/*
  	...
  	// data and the p.d.f in the frame
        RooDataSet* data = novo.generate(mbb,obs) ;
  	RooPlot* xframe = mbb.frame(Title("Novosibirsk PDF "+outname)) ;
  	data->plotOn(xframe,RooFit::Name("data_curve"));
  	novo.plotOn(xframe,RooFit::Name("fit_curve"));
	...
	c->SaveAs("Fitresult_"+outname+"_SR3.pdf");
*/
	return h1;
}
...
void drawcanvas(TH1* h_novo, TH1* h_syst)
{
	...
	canvas.SaveAs(pdfname+"_SR3_Dijet.pdf");

}


//int main(int argc, char* argv[])
void QCD_Templates_SR3_Dijet() 
{
	//TH1::SetDefaultSumw2();
 	// R e a d   w o r k s p a c e   f r o m   f i l e
  	// -----------------------------------------------
  	// Open input file with workspace
	//TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale/novosibirsk_500to1700_25GeV/workspace/FitContainer_workspace.root") ; //old WP
  	TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale_v2/dijetv2_500to1700_25GeV_G7/workspace/FitContainer_workspace.root") ; //new WP
	...
	w->Print("v");
  	...
  	RooRealVar* peak = w->var("par_a");
  	RooRealVar* tail = w->var("par_b");
  	RooRealVar* width = w->var("par_c");
 	...
 	TH1* h_central = novopsprod->createHistogram("QCD", *mbb, Binning(48,500.,1700.));
	...
	//data->SetName("data_obs");
  	...
  	// Make a second plot frame in x and draw both the 
  	// data and the p.d.f in the frame
  	RooPlot* xframe = mbb->frame(Title("Dijet PDF with Pseudo-Data in SR")) ;
  	data->plotOn(xframe,RooFit::Name("data_curve"));
  	...
  	gPad->SetLeftMargin(0.15); xframe->GetYaxis()->SetTitleOffset(1.6); xframe->Draw();
  	c->SaveAs("Fitresult_Central_SR3_Dijet.pdf");
	...
	TH1* h_novo = (TH1*)h_central->Clone("h_novo");
  	...
  	TFile* outputFile = new TFile("QCD_Templates_SR3_Dijet.root","RECREATE");
  	outputFile->cd();
  	h_central->Write();
 	h_novo->Write();
	//data->Write();
	...
	// Declare new histogram with initialization to central one
	TH1* h_syst_up  = (TH1*)h_central->Clone("h_syst_up");
        TH1* h_syst_down= (TH1*)h_central->Clone("h_syst_down");
        TH1* h_tmp_1    = (TH1*)h_central->Clone("h_tmp_1");

  	h_syst_up = QCD_HistSyst(1,0,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,1.0);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
  	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,0,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,1.0);  
	...
	canvas->cd(2);
  	...
  	h_syst_up = QCD_HistSyst(1,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,2.0);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
        h_syst_up->Add(h_central,h_tmp_1,1.,-5.);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,2.0);
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
        h_syst_down->Add(h_central,h_tmp_1,1.,5.);
  	h_syst_down->SetLineColor(kBlue);
  	h_syst_down->SetLineWidth(2);
  	h_syst_down->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_down);
  	...
  	h_syst_up = QCD_HistSyst(1,2,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,2.0);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
        h_syst_up->Add(h_central,h_tmp_1,1.,-5.);
    	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,2,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,2.0);
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
        h_syst_down->Add(h_central,h_tmp_1,1.,5.);
  	h_syst_down->SetLineColor(kBlue);
  	h_syst_down->SetLineWidth(2);
	...
	drawcanvas(h_novo,h_syst_down);
  	...
  	canvas->SaveAs("QCD_Templates_SR3_Dijet.pdf");
