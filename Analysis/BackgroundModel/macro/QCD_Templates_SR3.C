...
*/
	...
	TH1* h1 = novo.createHistogram("QCD", mbb, Binning(48,500.,1700.));
...
void drawcanvas(TH1* h_novo, TH1* h_syst)
{
	...
	canvas.SaveAs(pdfname+"_SR3.pdf");

}


//int main(int argc, char* argv[])
void QCD_Templates_SR3() 
{
	//TH1::SetDefaultSumw2();
	...
	//TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale/novosibirsk_500to1700_25GeV/workspace/FitContainer_workspace.root") ; //old WP
  	TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale_v2/novosibirsk_500to1700_25GeV_G7/workspace/FitContainer_workspace.root") ; //new WP
	...
	w->Print("v");
  	...
  	RooRealVar* peak = w->var("peak1");
  	RooRealVar* tail = w->var("tail1");
  	RooRealVar* width = w->var("width1");
	...
	//data->SetName("data_obs");
  	...
  	// Make a second plot frame in x and draw both the 
  	// data and the p.d.f in the frame
  	RooPlot* xframe = mbb->frame(Title("Novosibirsk PDF with Pseudo-Data in SR")) ;
  	data->plotOn(xframe,RooFit::Name("data_curve"));
  	...
  	gPad->SetLeftMargin(0.15); xframe->GetYaxis()->SetTitleOffset(1.6); xframe->Draw();
  	c->SaveAs("Fitresult_Central_SR3.pdf");
	...
	TH1* h_novo = (TH1*)h_central->Clone("h_novo");
  	...
  	TFile* outputFile = new TFile("QCD_Templates_SR3.root","RECREATE");
  	outputFile->cd();
  	h_central->Write();
 	h_novo->Write();
	//data->Write();
	...
	TH1* h_syst_up  = (TH1*)h_central->Clone("h_syst_up");
  	...
  	h_syst_up = QCD_HistSyst(1,0,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,0.5);
	h_tmp_1->Add(h_syst_up,h_central,1.,-1.); 	//red is above
	h_syst_up->Add(h_central,h_tmp_1,1.,3.);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
  	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,0,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,0.5);  
	h_tmp_1->Add(h_central,h_syst_down,1.,-1.);	//blue is below
	h_syst_down->Add(h_central,h_tmp_1,1.,-3.);
	...
	canvas->cd(2);
  	...
  	h_syst_up = QCD_HistSyst(1,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,0.08);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
        h_syst_up->Add(h_central,h_tmp_1,1.,-100.);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,0.08);
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
        h_syst_down->Add(h_central,h_tmp_1,1.,100.);
  	h_syst_down->SetLineColor(kBlue);
  	h_syst_down->SetLineWidth(2);
  	h_syst_down->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_down);
	...
	drawcanvas(h_novo,h_syst_down);
  	...
  	canvas->SaveAs("QCD_Templates_SR3.pdf");
