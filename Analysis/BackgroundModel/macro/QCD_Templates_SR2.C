...
//const double obs = 34757.;	
//const double obs = 147630.; 	//Full re-reco old WP
const double obs = 105053.; 	//new WP 
...
TH1* QCD_HistSyst(bool up, int iPar, double init_3[3], double para_a_array[3], double para_b_array[3], double dpara_b_array[3], TMatrixD para_b, TMatrixD eigenVa, double n)
{
	...
	TH1* h1 = novo.createHistogram("QCD", mbb, Binning(42,350.,1190.));
...
/*
	...
	c->SaveAs("Fitresult_"+outname+"_SR2.pdf");
*/
	return h1;
}
...
void drawcanvas(TH1* h_novo, TH1* h_syst)
{
	...
	canvas.SaveAs(pdfname+"_SR2.pdf");

}


//int main(int argc, char* argv[])
void QCD_Templates_SR2() 
{
	//TH1::SetDefaultSumw2();
 	// R e a d   w o r k s p a c e   f r o m   f i l e
  	// -----------------------------------------------
  	// Open input file with workspace
	//TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale/novosibirsk_350to1190_20GeV/workspace/FitContainer_workspace.root") ; //old WP  
	TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale_v2/novosibirsk_350to1190_20GeV_G7/workspace/FitContainer_workspace.root") ; //new WP
	...
	w->Print("v");
 	...
 	TH1* h_central = novopsprod->createHistogram("QCD", *mbb, Binning(42,350.,1190.));
	...
	//data->SetName("data_obs");
  	...
  	gPad->SetLeftMargin(0.15); xframe->GetYaxis()->SetTitleOffset(1.6); xframe->Draw();
  	c->SaveAs("Fitresult_Central_SR2.pdf");
	...
	TH1* h_novo = (TH1*)h_central->Clone("h_novo");
  	...
  	TFile* outputFile = new TFile("QCD_Templates_SR2.root","RECREATE");
  	outputFile->cd();
  	h_central->Write();
 	h_novo->Write();
	//data->Write();
	...
	//TH1* h_tmp_2 	= (TH1*)h_central->Clone("h_tmp_2");
	...
	canvas->cd(2);
  	...
  	h_syst_up = QCD_HistSyst(1,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,1.0);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
        h_syst_up->Add(h_central,h_tmp_1,1.,-20.);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,1.0);
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
        h_syst_down->Add(h_central,h_tmp_1,1.,20.);
  	h_syst_down->SetLineColor(kBlue);
  	h_syst_down->SetLineWidth(2);
  	h_syst_down->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_down);
	...
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
        h_syst_up->Add(h_central,h_tmp_1,1.,-10.);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
	...
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
        h_syst_down->Add(h_central,h_tmp_1,1.,10.);
  	h_syst_down->SetLineColor(kBlue);
  	h_syst_down->SetLineWidth(2);
  	h_syst_down->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_down);
  	...
  	canvas->SaveAs("QCD_Templates_SR2.pdf");
