...
TH1* QCD_HistSyst(bool up, int iPar, double init_3[3], double para_a_array[3], double para_b_array[3], double dpara_b_array[3], TMatrixD para_b, TMatrixD eigenVa_inverse)
{
  	TString outname; 
	...
	para_b_sys.Print("v");
	para_a_sys.Mult(eigenVa_inverse,para_b_sys);	// a+/-da = T-1(b+/-db)
 	para_a_sys.Print("v");
	...
	c->SaveAs("Fitresult_"+outname+".pdf");
...
void QCD_Templates() 
{
  ...
  TFile* outputFile = new TFile("QCD_Templates.root","RECREATE");
  outputFile->cd();
  h_central->Write();
  ...
  double cm_elements[9] = {1.,-0.6704,-0.782,-0.6704,1.,0.9579,-0.782,0.9579,1.}; // correlation matrix
  //double cm_elements[9] = {3.081,-0.01921,-1.63,-0.01921,0.0002664,0.01857,-1.63,0.01857,1.41}; // covariance matrix
  double eigen_elements[9] = {-0.5376,0.8235,-0.1813,0.5844,0.5189,0.6238,0.6078,0.2294,-0.7602}; // corr eigen
  //double eigen_elements[9] = {-0.8532,0.5216,0.001885,0.006394,0.01407,-0.9999,0.5215,0.8531,0.01534}; // cov eigen
  ...
  para_b.Mult(eigenVa_inverse,para_a);
  dpara_b.Mult(eigenVa_inverse,dpara_a);
  ...
  canvas->cd(2);
  //gPad->SetLogy();
  h_central->GetYaxis()->SetRangeUser(0,2400.);
  h_central->Draw("hist");
  ...
  canvas->SaveAs("QCD_Templates.pdf");
