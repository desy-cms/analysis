...
TH1* QCD_HistSyst(bool up, int iPar, double init_3[3], double para_a_array[3], double para_b_array[3], double dpara_b_array[3], TMatrixD para_b, TMatrixD eigenVa, double n)
{
	...
	RooRealVar mbb("mbb","mbb", 200.0, 650.0, "GeV") ;
	...
	//mbb.setBins(73);
 	// Build PS*Novosibirsk p.d.f in terms of mbb,peak,width and tail
  	...
  	RooFormulaVar eff("eff", "0.5*(TMath::Erf(0.0166748 * (mbb - 245.507)) + 1)", RooArgList(mbb));
	RooEffProd novoeffprod("novoeffprod","novoeffprod PDF", novo, eff);

	TH1* h1 = novoeffprod.createHistogram("QCD", mbb, Binning(45,200.,650.));
...
/*
	...
	c->SaveAs("Fitresult_"+outname+"_2015.pdf");
*/
	return h1;
}
...
void drawcanvas(TH1* h_novo, TH1* h_syst)
{
	...
	canvas.SaveAs(pdfname+"_SR1_NovoEff.pdf");

}


//int main(int argc, char* argv[])
void QCD_Templates_SR1_NovoEfffix() 
{
	//TH1::SetDefaultSumw2();
 	// R e a d   w o r k s p a c e   f r o m   f i l e
  	// -----------------------------------------------
  	// Open input file with workspace
  	TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale_v2/novoefffixprod_200to650_10GeV_G7/workspace/FitContainer_workspace.root") ;
	...
	//data->SetName("data_obs");
  	...
  	gPad->SetLeftMargin(0.15); xframe->GetYaxis()->SetTitleOffset(1.6); xframe->Draw();
  	c->SaveAs("Fitresult_Central_SR1_NovoEff.pdf");
	...
	TH1* h_novo = (TH1*)h_central->Clone("h_novo");
  	...
  	TFile* outputFile = new TFile("QCD_Templates_SR1_NovoEff.root","RECREATE");
  	outputFile->cd();
  	h_central->Write();
 	h_novo->Write();
	//data->Write();
	...
	TH1* h_syst_up  = (TH1*)h_central->Clone("h_syst_up");
  	...
  	h_syst_up = QCD_HistSyst(1,0,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,5.0);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
	h_syst_up->Add(h_central,h_tmp_1,1.,-9.);
  	h_syst_up->SetLineColor(kRed);
  	h_syst_up->SetLineWidth(2);
  	h_syst_up->SetStats(kFALSE);
  	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,0,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,5.0);  
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
	h_syst_down->Add(h_central,h_tmp_1,1.,9.);
	...
	canvas->cd(2);
  	...
  	h_syst_up = QCD_HistSyst(1,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,5.0);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
  	...
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,1,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,5.0);
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
	...
	drawcanvas(h_novo,h_syst_down);
  	...
  	h_syst_up = QCD_HistSyst(1,2,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,5.0);
	h_tmp_1->Add(h_central,h_syst_up,1.,-1.);
  	...
  	h_syst_up->SetStats(kFALSE);
	drawcanvas(h_novo,h_syst_up);
  	h_syst_down = QCD_HistSyst(0,2,init_3,para_a_array,para_b_array,dpara_b_array,para_b,eigenVa,5.0);
	h_tmp_1->Add(h_syst_down,h_central,1.,-1.);
	...
	drawcanvas(h_novo,h_syst_down);
  	...
  	canvas->SaveAs("QCD_Templates_SR1_NovoEff.pdf");
