...
//const double binning[47] = {240.,260.,280.,300.,320.,340.,360.,380.,400.,420.,440.,460.,480.,500.,520.,540.,560.,580.,600.,620.,640.,660.,680.,700.,720.,740.,760.,780.,800.,850.,900.,950.,1000.,1050.,1100.,1150.,1200.,1250.,1300.,1350.,1400.,1450.,1500.,1550.,1600.,1650.,1700.};
...
TH1* QCD_HistSyst(bool up, int iPar, double init_3[3], double para_a_array[3], double para_b_array[3], double dpara_b_array[3], TMatrixD para_b, TMatrixD eigenVa)
{
	...
	RooRealVar mbb("mbb","mbb", 240.0, 1700.0, "GeV") ;
...
void drawcanvas(TH1* h_novo, TH1* h_syst)
{
	...
	canvas.SaveAs(pdfname+"_2015.pdf");

}


//int main(int argc, char* argv[])
void QCD_Templates_2015() 
{
	//TH1::SetDefaultSumw2();
 	// R e a d   w o r k s p a c e   f r o m   f i l e
  	// -----------------------------------------------
  	// Open input file with workspace
  	TFile *f = new TFile("/afs/desy.de/user/c/chayanit/private/MSSMHBB_Myfork/CMSSW_7_6_3/src/Analysis/BackgroundModel/test/Prescale/2015_novopsprod_240to1700_20GeV/workspace/FitContainer_workspace.root") ;
	...
	//data->SetName("data_obs");
  	...
  	// Make a second plot frame in x and draw both the 
  	// data and the p.d.f in the frame
  	RooPlot* xframe = mbb->frame(Title("PS x Novosibirsk PDF with Pseudo-Data in 2015")) ;
  	data->plotOn(xframe,RooFit::Name("data_curve"));
  	...
  	gPad->SetLeftMargin(0.15); xframe->GetYaxis()->SetTitleOffset(1.6); xframe->Draw();
  	c->SaveAs("Fitresult_Central_2015.pdf");
	...
	TH1* h_novo = (TH1*)h_central->Clone("h_novo");
  	...
  	TFile* outputFile = new TFile("QCD_Templates_2015.root","RECREATE");
  	outputFile->cd();
  	h_central->Write();
 	h_novo->Write();
	//data->Write();
	...
	drawcanvas(h_novo,h_syst_down);
  	...
  	canvas->SaveAs("QCD_Templates_2015.pdf");
