...
def submit_batch_jobs(splitted_configs, out_dir):
    ...
    for configs in splitted_configs:
        ...
        file_name = op.join(submit_dir,
                            "submit_background_model_massscan_"+
                            str(i).zfill(4)+".sh")
...
def create_job_configs(out_dir):
    ...
    inputs = {#"8TeV":  ["--input_file",
              ...
              "13TeV": ["--input_tree_file",
                        op.join(data_dir, "TripleBTagReverseSelection_13TeV_unblind.root")]}
    models = {"polynomials": [""], #"bernstein", "berneffprod"], #"Chebychev"
              "non-polynomials": ["novosibirsk", "novopsprod", "novoeffprod", "gausexp"]}
    fit_min = [str(x) for x in xrange(200, 251, 5)]
    fit_max = [str(x) for x in xrange(2000, 2001, 50)]
