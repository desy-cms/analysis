void plot_eff_pt3_mw()
{
   ...
   for ( int i = 0; i < 3; ++i )
   {
      h_pt[i]    = (TH1F*) fs[i] -> Get("h_PtJet_2");
      h_pt[i]    -> SetName(Form("h_PtJet3_S_%i",i));
      h_pt[i+3]  = (TH1F*) fb[i] -> Get("h_PtJet_2");
      h_pt[i+3]  -> SetName(Form("h_PtJet3_B_%i",i));
   }
   ...
   g[0] -> GetXaxis() -> SetTitle("pT_3 cut (GeV)");
...
//   leg->SetHeader("The Legend Title");
   ...
   c1 -> SaveAs("figs/j3ptcut_mw_eff-"+selection+".png");
   ...
   gsob[0] -> GetXaxis() -> SetTitle("pT_3 cut (GeV)");
   gsob[0] -> Draw("APC");
   gsob[1] -> Draw("PC");
   gsob[2] -> Draw("PC");
   gsig[0] -> Draw("PC");
...
//   leg->SetHeader("The Legend Title");
   ...
   c2 -> SaveAs("figs/j3ptcut_mw_sig-"+selection+".png");
