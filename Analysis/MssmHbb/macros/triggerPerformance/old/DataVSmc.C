...
int DataVSmc2(){
   ...
   TH1 *nullH = nullptr;
   TLegend *leg = new TLegend(0.5,0.3,0.8,0.75);
   ...
   //........................PT.....................................//
	...
	pf100Eff_bg_num->Divide(pf100Eff_bg_num,pf100Eff_bg_den,1,1,error_mode.c_str());
	pf100Eff_bg_num->SetMarkerStyle(21);
	pf100Eff_bg_num->SetMarkerColor(kMagenta);
   TH1D *pf100Eff_signal_mc[15];
   ...
   leg->AddEntry(pf100Eff_bg_num,"Bg MC","l");
   pf100Eff_bg_num->SetLineWidth(1.5);
   pf100Eff_bg_num->Draw("same histo");
   leg->Draw();
   oneDFitLowM->Print("../pictures/TriggerPerformance/1DEff_QCD_PF80_PF100_lowM_pt1.pdf");


   //........................ETA....................................//
   TCanvas *oneDFitLowM_eta = new TCanvas("oneDFitLowM_eta","Factor Low M ETa",1000,800);
	TH1D *pf100Eff_eta = (TH1D*) fDataLowM->Get("TriggerEfficiencies/KinTrigEff_1D_Pf80_Pf100_eta1");
	pf100Eff_eta->SetMarkerStyle(20);
	TH1D *pf100Eff_eta_bg_num = (TH1D*) BgMC->Get("TriggerEfficiencies/KinTrigEff_Num_1D_Pf80_Pf100_eta1");
	TH1D *pf100Eff_eta_bg_den = (TH1D*) BgMC->Get("TriggerEfficiencies/KinTrigEff_Den_1D_Pf80_Pf100_eta1");
	pf100Eff_eta_bg_num->Divide(pf100Eff_eta_bg_num,pf100Eff_eta_bg_den,1,1,error_mode.c_str());
	pf100Eff_eta_bg_num->SetMarkerStyle(21);
	pf100Eff_eta_bg_num->SetMarkerColor(kMagenta);
	TH1D *pf100Eff_eta_signal_mc[15], *pf100Eff_eta_signal_mc_scale[15];
	i = 0;
	for(const auto & f : SignalMC){
	   ++i;
	   pf100Eff_eta_signal_mc[i] = (TH1D*) f.second->Get("TriggerEfficiencies/KinTrigEff_1D_Pf80_Pf100_eta1");
	   pf100Eff_eta_signal_mc[i]->SetMarkerColor(50+3*i);
	   pf100Eff_eta_signal_mc[i]->SetMarkerStyle(20);
	   pf100Eff_eta_signal_mc[i]->Draw("same E");
	   pf100Eff_eta->Draw("same E");
   }
	pf100Eff_eta_bg_num->SetLineWidth(1.5);
	pf100Eff_eta_bg_num->Draw("same histo");
	leg->Draw();
	oneDFitLowM_eta->Print("../pictures/TriggerPerformance/1DEff_QCD_PF80_PF100_lowM_eta1.pdf");

	//........................ETA scaled to 1....................................//
	TCanvas *oneDFitLowM_eta_scaled = new TCanvas("oneDFitLowM_eta_scaled","Factor Low M ETa",1000,800);
	TH1D *pf100Eff_eta_scale = new TH1D(*pf100Eff_eta);
	pf100Eff_eta_scale->Scale(1./pf100Eff_eta_scale->Integral());
	TH1D *pf100Eff_eta_bg_num_scale = new TH1D(*pf100Eff_eta_bg_num);
	pf100Eff_eta_bg_num_scale->Scale(1./pf100Eff_eta_bg_num_scale->Integral());
	i=0;
	for(const auto & f : SignalMC){
		++i;
		pf100Eff_eta_signal_mc_scale[i] = new TH1D(*pf100Eff_eta_signal_mc[i]);
		pf100Eff_eta_signal_mc_scale[i]->Scale(1./pf100Eff_eta_signal_mc_scale[i]->Integral());
		pf100Eff_eta_signal_mc_scale[i]->Draw("same E");
		pf100Eff_eta_scale->Draw("same E");
	}
	pf100Eff_eta_bg_num_scale->Draw("same histo");
	leg->Draw();
	oneDFitLowM_eta_scaled->Print("../pictures/TriggerPerformance/1DEff_QCD_PF80_PF100_lowM_eta1_normalized.pdf");
	...
	TH1D *ratio_of_eta = (TH1D*) ratio.DrawRatio(pf100Eff_eta,pf100Eff_eta_bg_num,nullH,legenda,oneDLowM_eta_comp);
	...
	for(const auto & f: pTHat){
		...
		ptHatEtaLeg->AddEntry(ptHatEta1[i],(f.first + " QCD MC").c_str(),"p");
	...
	TH2D *qcd_num_pf80for100_pt1eta1 = (TH2D*) BgMC ->Get("TriggerEfficiencies/KinTrigEff_Num_2D_PF80_PF100_pt1eta1");
	TH2D *qcd_den_pf80for100_pt1eta1 = (TH2D*) BgMC ->Get("TriggerEfficiencies/KinTrigEff_Den_2D_PF80_PF100_pt1eta1");
	qcd_num_pf80for100_pt1eta1->Divide(qcd_num_pf80for100_pt1eta1,qcd_den_pf80for100_pt1eta1);
