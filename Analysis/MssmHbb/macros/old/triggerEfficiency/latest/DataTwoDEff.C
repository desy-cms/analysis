...
void DataTwoDEff()
{
   ...
   //float Bins[] = {0.,90.,92.5,95.,97.5,100.,105,110,115.,120.,125.,130.,135.,140.,145,150,170,200,250,360,500};
   ...
   for(int binx = 1; binx <= size; binx++)
   {
	   for(int biny = 1; biny <= size; biny++)
	   {
		   if(binx == biny){
			   ...
			   TwoDEffRefMC_NumSym->SetBinContent(binx,biny,0);
			   TwoDEffRefMC_NumSym->SetBinError(binx,biny,0);
		   }
	   }
   }
...
/*
   ...
   TH1D* bin2 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P0_{T2}, [GeV]",2,2,"e");
   bin2 -> SetMarkerColor(1);
   TH1D* bin4 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P2_{T2}, [GeV]",4,4,"e");
   bin4 -> SetMarkerColor(2);
   TH1D* bin6 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P3_{T2}, [GeV]",6,6);
   bin6 -> SetMarkerColor(4);
   TH1D* bin8 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P_{T2}, [GeV]",8,8);
   ...
   legP28 -> AddEntry(bin2,"Pt1=90-96","p");
   legP28 -> AddEntry(bin4,"Pt1=112-118","p");
   legP28 -> AddEntry(bin6,"Pt1=124-130","p");
   legP28 -> AddEntry(bin8,"Pt1=135-140","p");
   legP28 -> Draw();
   first4 ->cd();
   ...
   TH1D* bin10 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P4_{T2}, [GeV]",10,10,"e");
   bin10 -> SetMarkerColor(1);
   TH1D* bin12 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P5_{T2}, [GeV]",12,12,"e");
   bin12 -> SetMarkerColor(2);
   TH1D* bin14 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P6_{T2}, [GeV]",14,14);
   bin14 -> SetMarkerColor(4);
   TH1D* bin16 = (TH1D*) TwoDEffRefMC_Num -> ProjectionX("P7_{T2}, [GeV]",15,15);
   ...
   legP1016 -> AddEntry(bin10,"Pt1=145-150","p");
   legP1016 -> AddEntry(bin12,"Pt1=170-200","p");
   legP1016 -> AddEntry(bin14,"Pt1=250-360","p");
   legP1016 -> AddEntry(bin16,"Pt1=360-500","p");
   legP1016 -> Draw();
   ...
   //Check interpolation:
   float Bins2[] = {0.,90.,96.,112.,118.,124,130.,135.,140.,145,150,170,200,250,360,495};
   TCanvas *inter_canva = new TCanvas("inter_canva","interpolation Check",1000,800);
   TH2F *interp = new TH2F("interp","interpolation 2D",size,Bins2,size,Bins2);
   interp->SetMarkerStyle(20);
   interp ->SetBinContent(0,0,0);
   for(int binx = 1; binx < size; binx++)
   {
	   for(int biny = 1; biny < size; ++biny)
	   {
		   //std::cout<<"Val: "<<TwoDEffRefMC_Num->Interpolate(Bins2[binx],Bins2[biny])<<" binx = "<<binx<<" "<<Bins2[binx]<<" biny = "<<biny<<" "<<Bins2[biny]<<std::endl;
		   interp ->SetBinContent(binx+1,biny+1,TwoDEffRefMC_Num->Interpolate(Bins2[binx] + (Bins2[binx+1]-Bins2[binx])/2,Bins2[biny]+ (Bins2[biny+1]-Bins2[biny])/2));
		   //std::cout<<"Pt2 = "<<Bins[binx]<<" Pt1 = "<<Bins[biny]<<" Comp = "<<TwoDEffRefMC_Num ->GetBinContent(binx+1,biny+1)<<" "<<TwoDEffRefMC_Num->Interpolate(Bins2[binx],Bins2[biny])<<std::endl;
		   //interp->SetBinContent(binx+1,biny+1,TwoDEffRefMC_Num ->GetBinContent(binx+1,biny+1)/TwoDEffRefMC_Num->Interpolate(Bins2[binx],Bins2[biny]));
	   }
   }

   interp->Divide(TwoDEffRefMC_Num);
   interp->Draw("E");
...
//   f->Close();
   ...
   //Compare 2D and 1D*1D efficiency
   TCanvas *eff_comp = new TCanvas("eff_comp","Comparison Of 2F and 1D*1D eff",1000,800);
   TH2F *OneDimTimesOneDim = new TH2F("OneDimTimesOneDim","",size,Bins,size,Bins);
   OneDimTimesOneDim->SetMarkerStyle(20);


   TH1F *EffJ1 = new TH1F("EffJ1","1D Efficiency with Ref method",size,Bins);
   EffJ1 ->SetMarkerStyle(20);
   TH1F *EffJ1_denum = new TH1F("EffJ1_denum","1D Efficiency with Ref method",size,Bins);
   TwoDEffRefMC_NumSym ->SetTitle("1D Efficiency with Ref method;p_{T2};p_{T1}");

   TH1F *EffJ2 = new TH1F("EffJ2","1D Efficiency with Ref method",size,Bins);
   EffJ2 ->SetMarkerStyle(20);
   TH1F *EffJ2_denum = new TH1F("EffJ2_denum","1D Efficiency with Ref method",size,Bins);
   TwoDEffRefMC_NumSym ->SetTitle("1D Efficiency with Ref method;p_{T2};p_{T1}");

   EffJ1->Multiply(EffJ2);
   double value;
   char cutJ1[200], cutJ2[200], cutJ1Denum[200], cutJ2Denum[200];
   for(int pt1 = 1; pt1 < size; pt1++)
   {

	   for(int pt2 = 1; pt2 < size; ++pt2)
	   {
		   sprintf(cutJ1,"LeadMatch100[0] && dPhiFS>2.7  && LeadPt[1] > %f && LeadPt[1] < %f",Bins[pt2],Bins[pt2+1]);
		   sprintf(cutJ1Denum," dPhiFS>2.7  && LeadPt[1] > %f && LeadPt[1] < %f",Bins[pt2],Bins[pt2+1]);
		   sprintf(cutJ2,"LeadMatch100[1] && dPhiFS>2.7 && LeadPt[0] > %f && LeadPt[0] < %f ",Bins[pt1],Bins[pt1+1]);
		   sprintf(cutJ2Denum," dPhiFS>2.7 && LeadPt[0] > %f && LeadPt[0] < %f",Bins[pt1],Bins[pt1+1]);

		   dataTree ->Draw("hpf100_Num[0]>>EffJ1",cutJ1);
		   dataTree ->Draw("hpf100_Denum[0]>>EffJ1_denum",cutJ1Denum);
		   EffJ1 ->Divide(EffJ1,EffJ1_denum,1,1,"b");

		   dataTree ->Draw("hpf100_Num[1]>>EffJ2",cutJ2);
		   dataTree ->Draw("hpf100_Denum[1]>>EffJ2_denum",cutJ2Denum);
		   EffJ2 ->Divide(EffJ2,EffJ2_denum,1,1,"b");
		   value = EffJ1->Interpolate(Bins[pt1] + (Bins[pt1+1]-Bins[pt1])/2) *
					EffJ2->Interpolate(Bins[pt2]+ (Bins[pt2+1]-Bins[pt2])/2);
		   std::cout<<"1D: "<<value<<" J1: "<<EffJ1->Interpolate(Bins[pt1] + (Bins[pt1+1]-Bins[pt1])/2)<<" "<<EffJ2->Interpolate(Bins[pt2]+ (Bins[pt2+1]-Bins[pt2])/2)<<std::endl;
		   OneDimTimesOneDim->SetBinContent(pt2+1,pt1+1,value/TwoDEffRefMC_Num->Interpolate(Bins2[pt2] + (Bins2[pt2+1]-Bins2[pt2])/2,Bins2[pt1]+ (Bins2[pt1+1]-Bins2[pt1])/2));
		   std::cout<<"pt1 = "<<Bins[pt1] + (Bins[pt1+1]-Bins[pt1])/2<<" pt2: "<<Bins[pt2]+ (Bins[pt2+1]-Bins[pt2])/2<<" Value: "<<value<<" "<<TwoDEffRefMC_Num->Interpolate(Bins2[pt2] + (Bins2[pt2+1]-Bins2[pt2])/2,Bins2[pt1]+ (Bins2[pt1+1]-Bins2[pt1])/2)<<std::endl;
	   }
   }
   OneDimTimesOneDim->Draw("");
