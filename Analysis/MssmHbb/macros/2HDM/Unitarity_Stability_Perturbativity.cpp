...
void TwoDPlot_tanB_vs_cosB_A(TFile& file, const vector<double>& sinB_As, const string& type = "type3");
...
//	Histo with 2HDM results:
	...
	for(const auto& type : types){
		string thdm_fname = "/nfs/dust/cms/user/shevchen/SusHiScaner/output/production_corseBins_cosB_A_-1_1_tanB_1-100/rootFiles/" + type + "_mA.root";
		TFile f(thdm_fname.c_str(),"read");
		// Get TTree:
		TTree& tree = (TTree&)* f.Get("tree");
		// Call functions to draw phase-space
//		TwoDPlot_tanB_vs_sinB_A(tree,mAs,"unitarity && stability && perturbativity",type);
		TwoDPlot_tanB_vs_cosB_A(tree,mAs,type);
	}
