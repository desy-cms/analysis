# /users/rwalsh/run2/mssmhbb/scratch/V56 (CMSSW_7_4_2_HLT3)

import FWCore.ParameterSet.Config as cms

process = cms.Process( "TEST" )

process.load('setup_740_GRun_V61_cff')

process.HLTConfigVersion = cms.PSet(
  tableName = cms.string('/users/rwalsh/run2/mssmhbb/scratch/V56')
)
...
process.hltL1sL1SingleJetX = cms.EDFilter( "HLTLevel1GTSeed",
...
process.hltPreL1SingleJetX = cms.EDFilter( "HLTPrescaler",
    L1GtReadoutRecordTag = cms.InputTag( "hltGtDigis" ),
    offset = cms.uint32( 0 )
)
...
process.hltPrePFBJetXBTag0pX = cms.EDFilter( "HLTPrescaler",
    L1GtReadoutRecordTag = cms.InputTag( "hltGtDigis" ),
    offset = cms.uint32( 0 )
)
...
process.hltSingleCaloBJetX = cms.EDFilter( "HLT1CaloJet",
    saveTags = cms.bool( True ),
...
process.hltCSVL30pX = cms.EDFilter( "HLTCaloJetTag",
    saveTags = cms.bool( True ),
    MinJets = cms.int32( 1 ),
...
process.hltPFBJetsCorrectedMatchedToCaloBJets40 = cms.EDProducer( "PFJetsMatchedToFilteredCaloJetsProducer",
    DeltaR = cms.double( 0.5 ),
    CaloJetFilter = cms.InputTag( "hltSingleCaloBJetX" ),
    TriggerType = cms.int32( 86 ),
    PFJetSrc = cms.InputTag( "hltAK4PFJetsCorrectedLooseID" )
)
process.hltSinglePFBJetX = cms.EDFilter( "HLT1PFJet",
    saveTags = cms.bool( True ),
    MinPt = cms.double( 20.0 ),
    MinN = cms.int32( 1 ),
    MaxEta = cms.double( 5.0 ),
    MinMass = cms.double( -1.0 ),
    inputTag = cms.InputTag( "hltPFBJetsCorrectedMatchedToCaloBJets40" ),
    MinE = cms.double( -1.0 ),
    triggerType = cms.int32( 86 )
)
...
process.hltCSVPF0pX = cms.EDFilter( "HLTPFJetTagWithMatching",
    saveTags = cms.bool( True ),
...
process.HLT_L1SingleJetX_v2 = cms.Path( process.HLTBeginSequence + process.hltL1sL1SingleJetX + process.hltPreL1SingleJetX + process.HLTEndSequence )
process.HLT_PFBJetX_BTag0pX_v2 = cms.Path( process.HLTBeginSequence + process.hltL1sL1ZeroBias + process.hltPrePFBJetXBTag0pX + process.HLTAK4CaloJetsSequence + process.hltSingleCaloBJetX + process.HLTFastPrimaryVertexSequence + process.HLTBtagCSVSequenceL3 + process.hltCSVL30pX + process.HLTAK4PFJetsSequence + process.hltAK4PFJetsCorrectedLooseID + process.hltPFBJetsCorrectedMatchedToCaloBJets40 + process.hltSinglePFBJetX + process.HLTBtagCSVSequencePF + process.hltSelector6PFJets + process.hltCSVPF0pX + process.HLTEndSequence )
...
process.HLTSchedule = cms.Schedule( *(process.HLTriggerFirstPath, process.HLT_L1SingleJetX_v2, process.HLT_PFBJetX_BTag0pX_v2, process.HLTriggerFinalPath ))
