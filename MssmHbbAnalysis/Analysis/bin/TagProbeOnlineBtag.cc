...
//   filename = boost::filesystem::basename(filename);
   ...
   for ( int i = 0 ; i < nMeta ; ++i )
   {
      ...
      nGenTotal_ += nGenTotal_;
      nGenFilt_  += nGenFilt_;
   }
   
   double genEff = nGenTotal_ > 0 ? (double) nGenFilt_ / (double) nGenTotal_ : 1.;
   
   xsection_ *= 1000; // convert from pb to fb
   double effXsection = xsection_ * genEff;
...
//    
   ...
   double effLumi = nEvents/effXsection;
   double scale   = lumi/effLumi;
   
   std::cout << "Cross section   = " << effXsection << std::endl;
   std::cout << "Effective lumi  = " << effLumi   << std::endl;
   std::cout << "Number of events = " << nEvents*scale << std::endl;
